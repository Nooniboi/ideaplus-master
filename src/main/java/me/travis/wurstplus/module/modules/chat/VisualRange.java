        ...
        if (tickPlayerList.size() > 0) {
            for (String playerName : tickPlayerList) {
                ...
                if (this.publicChat.getValue().booleanValue()) {
                    VisualRange.mc.player.connection.sendPacket((Packet)new CPacketChatMessage("here comes " + playerName + ", lol they r kinda bad tbh?"));
                } else if (Friends.isFriend(playerName)) {
                    Command.sendChatMessage("\u00A76" + playerName + "\u00a7r" + " is here to help. #eztime");
                } else {
                    Command.sendChatMessage("\u00A73" + playerName + "\u00a7r" + " is here to #die. lol");
                }
                return;
            }
        }
        if (this.knownPlayers.size() > 0) {
            for (String playerName : this.knownPlayers) {
                ...
                if (this.leaving.getValue().booleanValue()) {
                    if (this.publicChat.getValue().booleanValue()) {
                        VisualRange.mc.player.connection.sendPacket((Packet)new CPacketChatMessage("lol " + playerName + " run more noob"));
                    } else if (Friends.isFriend(playerName)) {
                        Command.sendChatMessage("\u00A76" + playerName + "\u00a7r" + " has left, uh oh");
                    } else {
                        Command.sendChatMessage("\u00A73" + playerName + "\u00a7r" + " has gone, #ezmode amirite?");
                    }
                }
                return;
            }
